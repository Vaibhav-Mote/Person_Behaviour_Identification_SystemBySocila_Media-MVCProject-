...
public class PredictionRepositoryImpl  implements PredictionRepository{
	...
	public int getPostId(int postid, int predictUserID) {
        try {
            String sql = "SELECT pm.postid FROM postmaster pm " +
            ...
            Integer postid1 = template.queryForObject(sql, new Object[]{postid, predictUserID}, Integer.class);
            
            return (postid1 != null) ? postid1 : 0;
        } catch (Exception e) {
	...
	public String[] getAllPostsCommentsLikes(int userId) {
        List<String> results = new ArrayList<String>();

        // Fetch all posts
        String postSql = "SELECT pm.postname FROM postmaster pm " +
                         "INNER JOIN postregistrationjoin prj ON prj.postid = pm.postid " +
                         "INNER JOIN userregistrationmaster rm ON rm.registerid = prj.registerid " +
                         "WHERE rm.registerid = ?";
        results.addAll(template.queryForList(postSql, new Object[]{userId}, String.class));
int postcount=results.size();
        // Fetch all comments
        String commentSql = "SELECT comment FROM commentmaster WHERE registerid = ?";
        results.addAll(template.queryForList(commentSql, new Object[]{userId}, String.class));

        // Fetch liked posts
        String likedPostsSql = "SELECT pm.postname FROM likemaster lm " +
                               "INNER JOIN likepostjoin lpj ON lpj.likeid = lm.likeid " +
                               "INNER JOIN postmaster pm ON pm.postid = lpj.postid " +
                               "WHERE lm.registerid = ?";
        results.addAll(template.queryForList(likedPostsSql, new Object[]{userId}, String.class));
