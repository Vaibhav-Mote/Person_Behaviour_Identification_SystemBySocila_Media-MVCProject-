	 ...
	 @RequestMapping("likepost")
		public void isLikePost(HttpServletRequest request, HttpServletResponse response) throws IOException {
			...
			if (b) {
				...
				out.print("<a href='#' class='likeicon1234' id='likeicon1234" + postid
						+ "'><i class='bi bi-heart-fill text-danger' onclick='likeUnlikePost1234(" + postid + ")'></i>"
						+ likeCount + "</a>");
			} else {
				int likeCount = postService.isgetLikeCount(postid);
				out.print("<a href='#' class='likeicon1234' id='likeicon1234" + postid
						+ "'><i class='bi bi-heart-fill text-white' onclick='likeUnlikePost1234(" + postid + ")'></i>"
						+ likeCount + "</a>");
			}

		}
	 
	 @RequestMapping("comment1234")
...
System.out.println("ok");
	        ...
	        out.println("<a href='#' class='commenticon' id='commenticon1234" + postid + "'>" +
	                    "<i class='bi bi-chat-right'></i>" +
	                    commentCount + " <input id='comment1234" + postid + "' type='text' name='comment' value='' placeholder='Comment Here' style='width:7em;height:0px;padding:13px'/></a>");
	    }
